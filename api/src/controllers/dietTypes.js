...
let linkDiets = [
  //tomadas del enlace que se pasa en el readme y no vienen de la API
  { name: "vegetarian" },
...
async function getDiets(req, res, next) {
  try {
    ...
    if (dietSearch.length > 0) {
      ...
      let arrayDiets = response.data.results.map((recipe) => {
        //de cada receta tomo su arreglo de dietas
        return recipe.diets;
      });

      arrayDiets.forEach((dietArray) => {
        //por cada arreglo de dietas, las concateno
